...
async def parse_document(
    ...
    Returns: (markdown_text, page_numbers_per_block)
    """
        ...
        for _ in range(timeout // 2):
            ...
            if status == "completed":
                ...
                if result_resp.status_code == 200:
                    return _extract_markdown_with_pages(result_resp.json())
                break
            elif status == "failed":
...
async def _parse_sync(file_path: str, file_name: str, backend: str) -> tuple[str, list[int]]:
    """同步解析（兜底方案）"""
    base_url = settings.MINERU_API_URL
    file_bytes = Path(file_path).read_bytes()
    ...
    async with httpx.AsyncClient(timeout=settings.MINERU_TIMEOUT) as client:
        ...
        resp.raise_for_status()
        return _extract_markdown_with_pages(resp.json())


def _extract_markdown_with_pages(result: dict) -> tuple[str, list[int]]:
    """从 MinerU 响应中提取 Markdown + 页码列表"""
